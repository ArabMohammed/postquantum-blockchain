...
impl Server {
    ...
    pub fn new(port : &str, miner_address: &str, utxo : UTXOSet) -> Result<Server, Box<dyn std::error::Error>>{
    ...
    pub fn start_server(&self) -> Result<(),Box<dyn std::error::Error>>{
        ...
        info!("Start server at {}, mining address :{}",
        &self.node_address,&self.mining_address);
        thread::spawn(move||{
            thread::sleep(Duration::from_millis(1000));
            if server1.get_best_height().unwrap() == -1 {
                server1.request_blocks().unwrap()
            }else{
                server1.send_version(KNOWN_NODE1).unwrap()
            }
        });
        ...
        for stream in listener.incoming(){
            ...
            thread::spawn(move || server1.handle_connection(stream).unwrap());
        }
        Ok(())
    }
    ...
    /*************************************************************************************/
    /*************************************************************************************/
    fn send_block(&self, addr: &str, b: &Block) -> Result<(),Box<dyn std::error::Error>> {
    ...
    /***********************************************/
    fn send_inv(&self, addr: &str, kind: &str, items: Vec<String>) -> Result<(),Box<dyn std::error::Error>> {
        info!(
    ...
    /***********************************************/
    pub fn send_transaction(tx : &Transaction, utxoset : UTXOSet) -> Result<(),Box<dyn std::error::Error>>{
        let server = Server::new("7000","",utxoset)?;
        server.send_tx(KNOWN_NODE1,tx)?;
        Ok(())
    }
    /*******************************/
    pub fn send_tx(&self, addr: &str, tx: &Transaction) -> Result<(),Box<dyn std::error::Error>> {
    ...
    /************************************************/
    fn send_get_data(&self, addr: &str, kind: &str, id: &str) -> Result<(),Box<dyn std::error::Error>> {
        info!(
    ...
    fn handle_connection(&self, mut stream : TcpStream) -> Result<(),Box<dyn std::error::Error>>{
        ...
        info!("Accept request : lenght {}",count);
